...
int kty04_sign(groupsig_signature_t *sig, message_t *msg, groupsig_key_t *memkey,
  ...
  */
  ...
  for(aux_i=0; aux_i<kty04_sig->z; aux_i++) {
    ...
    /*   GOTOENDRC(IERROR, kty04_sign); */
    /* } */
    strcat(to_be_hashed, aux_sB);
    msg_len += strlen(aux_sB);
  ...
  for(aux_i=0; aux_i<kty04_sig->m; aux_i++) {
    ...
    /*   GOTOENDRC(IERROR, kty04_sign); */
    /* } */
    strcat(to_be_hashed, aux_sA);
    msg_len += strlen(aux_sA);
  ...
  aux_sc = (byte_t*) hash_message_hw((byte_t*) to_be_hashed, msg_len * 8);  // bits
  // debug messages
  /* if (aux_sc==NULL) printf("It is NULL\n"); */
  /* printf("Hash of sign: %s\n",aux_sc); */
  /* for(int i=0; i<15;i++){ */
  /*   printf("%x\n",aux_sc[i]); */
  /* } */
  /* printf("Message of sign: %s\n",to_be_hashed); */
  /* printf("Length of message in sign: %d\n",msg_len); */
