...
class TestGroupOps(TestCommon):
    ...
    def test_linkSignaturesSameUser(self):
        ...
        sig1 = groupsig.sign(msg, self.memkeys[0], self.grpkey, UINT_MAX)
        sig2 = groupsig.sign(msg, self.memkeys[0], self.grpkey, UINT_MAX)
        sigs = [sig1, sig2]
        msgs = [msg, msg]
    ...
    def test_linkSignaturesDifferentUser(self):
        ...
        msg = b"{ \"scope\": \"scp\", \"message\": \"Hello, World!\" }"
        sig1 = groupsig.sign(msg, self.memkeys[0], self.grpkey, UINT_MAX)
        sig2 = groupsig.sign(msg, self.memkeys[1], self.grpkey, UINT_MAX)
        sigs = [sig1, sig2]
        msgs = [msg, msg]
        gslink = groupsig.link(self.memkeys[0], self.grpkey, msg, sigs, msgs)
        proof = gslink['proof']
